...
> increment len by exactly 1 regardless of the space symbol's byte length, so a multi-byte space
> symbol has all but its first byte overwritten by the next append (latent bug).
...
> Note `[spec:foma:sem:apply.apply-append-fn]` advances the output position by only 1 byte per
> appended space symbol, so a multi-byte space symbol is corrupted in output (latent bug).
