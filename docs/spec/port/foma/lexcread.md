...
> - Otherwise the single leading character `rest[..c.len_utf8()]`, EXTENDED to swallow any immediately-following run of Unicode combining marks (`[spec:foma:sem:utf8.utf8iscombining-fn]`, the same check `apply.rs`'s tokenizer uses to merge such a run into one query-side token forced to IDENTITY), is interned as one symbol: looked up with `[spec:foma:sem:lexcread.lexc-find-sigma-hash-fn]`; on a miss, sigma_add'd to lexsigma and registered via `[spec:foma:sem:lexcread.lexc-add-sigma-hash-fn]`. Emit the number and advance past the whole run.
...
>
> DEVIATION from C (+2, deliberate divergence, not a memory-safety workaround): C foma and foma-rs 0.1.1-0.4.0 both tokenize a base character and any following combining mark(s) as SEPARATE one-codepoint symbols here, while `apply.rs`'s tokenizer (`[spec:foma:sem:apply.apply-create-sigmatch-fn]`'s sigmatch construction — see its own "a base symbol plus combining marks is one unknown symbol even when the base alone is in sigma (rationale: had base+mark been a sigma symbol, longest match would already have found it)" note, which is exactly the assumption this fix satisfies) unconditionally merges the same run in the QUERY string into one token forced to IDENTITY, which only ever matches a `?` (UNKNOWN) wildcard arc. A network with no declared multichar symbol for the run and no wildcard arc therefore has nothing that token can match: total, silent non-match for any word containing a base+combining-mark run — hit by any caller that NFD-normalizes source text before compiling lexc (e.g. an emitter mirroring HermitCrab's `Normalize(FormD)`), independent of affixation. This fork groups such a run into one symbol here too, so the compiled network gets one arc for it and `apply.rs`'s initial sigma-trie match (which runs before its own combining-merge check) consumes the whole run as an already-known symbol — the merge-check then finds nothing left over and never downgrades it to IDENTITY. An explicit `Multichar_Symbols` declaration for the same run, or for the base character alone, is checked first via the multichar-attempt step above and takes priority unchanged. Upstream (both C foma and divvun/foma-rs) keeps the original asymmetry under its bug-for-bug-with-upstream policy; this fork diverges intentionally since upstream will not.
