...
> Rewrite phase (terminate==0): first, if the start state (0) is itself not coaccessible (coacc[0]==0) the language is empty (no path from the start reaches a final): replace the line array with fsm_empty() (canonical empty machine — one non-final start), destroy net->sigma and replace it with a fresh sigma_create(), zero linecount/arccount/statecount, set is_pruned, free the temporaries, and return. This subsumes the no-final-states case (markcount==0 ⟹ coacc[0]==0). Wave 5 fix: C set mapping[0]=0 unconditionally ("state 0 always exists"), so a disconnected net whose start was pruned had its orphaned coaccessible component renumbered from 1, yielding a net with no start state; since a pruned start makes L empty regardless of any disconnected component, the empty machine is the correct result. Otherwise (state 0 coaccessible) build mapping: mapping[0]=0; for i=1..statecount-1, coaccessible states get consecutive numbers j=1,2,... in increasing old-number order.
...
> (The markcount==0 empty-language case is now handled up front by the coacc[0]==0 early return above, so state 0 is always coaccessible when this compaction runs.) Set net->linecount=new_linecount, net->arccount=new_arccount, net->statecount=markcount.
